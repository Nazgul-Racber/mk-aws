...
const Insıght = () => {
  return (
    <section className="container py-14 mx-auto">
      <div className=" ">
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8 ">
          <div>
            ...
            <p className="mt-4 text-gray-800 text-3xl font-semibold sm:text-2xl">
              We’re here to help and answer any question you might have, We look
              forward to hearing from you .
            </p>
          </div>
          <ul className="text-gray-700 font-semibold text-2xl grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8">
            {contactMethods.map((item, idx) => (
              <li key={idx} className="flex flex-col items-center">
                <div className="w-12 h-12 rounded-full border flex items-center justify-center text-gray-700">
                  {item.icon}
                </div>
                <h4 className="text-gray-800 text-lg font-medium xl:text-xl mt-2">
                  {item.title}
                </h4>
                <p className="text-center">{item.desc}</p>
                <a
                  href={item.link.href}
                  className="flex items-center gap-1 text-sm text-indigo-600 duration-150 hover:text-indigo-400 font-medium mt-2"
                >
                  {item.link.name}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
